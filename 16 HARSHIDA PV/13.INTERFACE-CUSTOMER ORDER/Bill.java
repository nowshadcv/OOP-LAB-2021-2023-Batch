	...
	public static void main(String[] args) {
		...
		for(int i = 0 ; i < n ; i++) {
			...
			for(int j = 0 ; j < m ; j++) {
				...
				int c = sc.nextInt();
				System.out.println("Enter product nit_Price: ");
				int d = sc.nextInt();
		...
		for(int i = 0; i < n; i++) {
			...
			for(int j =0; j<order[i].length; j++) {
				System.out.printf("%5s %20s %10s %10s",order[i][j].prod_id,order[i][j].name ,order[i][j].quantity, order[i][j].unit_price, order[i][j].total);
				System.out.println();}
			System.out.println("------------------------------------------------------");
